...
Java_ru_iu3_fclient_MainActivity_LogUsingJNI(JNIEnv *env, jclass, jstring str) {
...
Java_ru_iu3_fclient_MainActivity_decrypt(JNIEnv *env, jclass, jbyteArray key, jbyteArray data)
{
    ...
    //PKCS#5.
    int pad = buf[dsz - 1]; //количеству добавленных элементов и их значениям
    if (pad < 1 || pad > 8) {
        delete[] buf;
        env->ReleaseByteArrayElements(key, pkey, 0);
        env->ReleaseByteArrayElements(data, pdata, 0);
        return env->NewByteArray(0);  // Возвращаем пустой массив при ошибке
    }
    for (int i = dsz - pad; i < dsz; i++) {
        if (buf[i] != pad) {
            delete[] buf;
            env->ReleaseByteArrayElements(key, pkey, 0);
            env->ReleaseByteArrayElements(data, pdata, 0);
